function [SF,sS,SFM,mP,TraceSy]=SchmidTaylorEBSD(CS,ebsd,Misanglede,grains,Dirpath)
...
if length(grains)~=1
    ...
    for k=1:length(Operation)
        for i=1:length(ebsd.indexedPhasesId)
            ...
            Angles = orientation('map',Miller(sSGrains{k,i}.b,CS{ebsd.indexedPhasesId(i)}),...
                Miller(sSGrains{k,i}.n,CS{ebsd.indexedPhasesId(i)})).angle./degree;
            try
